# PEG 估值框架（v6 — 2026 年 6 月校準）

> 此檔由 `scripts/peg-review.mjs` 每月讀入，注入到 LLM prompt。
> 若分析共識升級到 v7、v8，**直接編輯這個檔即可**，不用改 code。
> peg-review 仍會保留 ±0.3 monthly change cap 與 confidence ≥0.7 安全閥。

## 1. 五因子評分方法論（從 Goldman / Bernstein / Morgan Stanley / JP Morgan / Mizuho 2026 共識萃取）
...
| 7.0 - 8.0 | 1.4 - 1.7 | AI 挑戰者、二線贏家、ABF 載板、ARM IP |
...
### 3.1 AI 需求方 Hyperscaler
- **NVIDIA** 全棧 + CUDA：**2.0 - 2.2**（v6 #1 score 9.75）
- **MSFT** OpenAI 獨家 + Azure：**1.7 - 1.9**
- **GOOGL** Cloud + Gemini + TPU，但 search 有 AI 替代風險：**1.5 - 1.7**

### 3.2 AI 晶片供應方
- **TSM** 製程獨占 + CoWoS 緊到 2027：**1.6 - 1.8**
- **ARM** AGI CPU 但 catalyst 多在 2027+：**1.7 - 1.9**
- **AMD** 全棧抗 NVIDIA 但 share 第二：**1.6 - 1.8**
- **記憶體 (MU / SK Hynix / Samsung)**：**1.4 - 1.6**（cyclical 折扣，HBM 超循環抵銷）
...
- **CEG** 24/7 firm power + CEO 鎖 20% EPS CAGR 到 2029：**1.7 - 1.9**
...
- **LYC.AX** 中國以外最大稀土：**1.7 - 1.9**（戰略受益 + 中國管制溢價）
- **MP** 美國本土唯一稀土：**1.7 - 1.9**
- **PLS.AX** 純鋰礦但鋰價 cyclical：**1.3 - 1.5**
...
| 事件 | 影響 PEG 方向 |
|---|---|
| **2026-11-27** 中國礦產管制截止日（鎵 / 鍺 / 鎢 / 銻） | 若管制重啟：稀土玩家 (LYC, MP) PEG ↑；CPO/光通訊 (MRVL, AVGO) PEG ↓ |
...
| 美國電網變壓器 5 年交期 + 50% 規劃 DC 可能延後 | AI 電力 (CEG, BE, VRT, GEV) backlog 越強 PEG 越能維持 |
...
**下次升級**：當分析報告 v7 出來時，直接編輯本檔（更新分類、調整錨點區間、補新 ticker），peg-review.mjs 下次跑就會自動套用，不需要改任何 code。
